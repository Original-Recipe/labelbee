...
// 将原本的some查找换成原始for循环查找，大数据量只做一次赋值的情况下，性能提升8倍
function calcRenderCuboidList(list, { x, y, z }, colorList, cb) {
  let newColorInfo;
  let found = false;
...
onmessage = function onmessage(e) {
  ...
  const { modifiedBoxIds = [], resetAreas = [] } = e.data;
  ...
  for (let i = 0; i < points.length; i += 3) {
    ...
    if (resetAreas.length === 1 && !modifiedBoxIds.length) {
      ...
      let isInReset = false;
      // 先找是否在选中的矩形坐标内
      const toRenderCuboidResult = calcRenderCuboidList(toRenderCuboidList, { x, y, z }, colorList, updateNum);
      ...
      if (found) {
        ...
        newColorInfo = REMAINED_COLOR_FLAG;
        const otherCuboidResult = calcRenderCuboidList(otherCuboidList, { x, y, z }, colorList, updateNum);
